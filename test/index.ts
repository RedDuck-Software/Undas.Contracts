...
import { MarketplaceV2, UndasGeneralNFT,Undas } from "../typechain";
...
describe("NftMarketplaceTest", function () {
  ...
  //   )).to.emit(NftMarketplace,'Listed')
    ...
    const collectionFactory = await ethers.getContractFactory("Undas");
    collection = await (await collectionFactory.deploy()).deployed();

    const marketplace_factory = await ethers.getContractFactory("MarketplaceV2")
    market = await marketplace_factory.deploy(  
      "0x87D7df5D9966DAE392ffE18abaC84232ff2e2806",
      "0x87D7df5D9966DAE392ffE18abaC84232ff2e2806",
      "0x87D7df5D9966DAE392ffE18abaC84232ff2e2806",
      20,
      10,
  ...
  it("allows to create collection => mit nft => allow to offer => allow to accept it",async function () {

     expect(await collection.connect(owner).createCollection("xdxd","xdxd","xdxd",2))
     await collection.connect(owner).safeMintGeneral(owner.address,"1st","1st","1st",0)
     
     console.log('first owner',await collection.ownerOf(0))
     console.log(await collection.tokenMetadata(0))

    await expect(market.connect(buyer).offerForNotListedToken(0,0,collection.address,{
      value: ethers.utils.parseUnits("2.0","ether")
    })).to.emit(market,"OfferForNotListed")

    await collection.connect(owner).setApprovalForAll(
      market.address,
      true
    )
     expect(await market.connect(owner).acceptOfferForNotListedToken(0,collection.address)).to.emit(market,"OfferForNotListed")
    .to.changeEtherBalance(owner,ethers.utils.parseUnits("2","ether"))
    
    console.log('second owner',await collection.ownerOf(0))

  })

  it("allows to create collection => mint nft => allow to offer => allows to deny it",async function () {
    expect(await collection.connect(owner).createCollection("xdxd","xdxd","xdxd",2))
   await collection.connect(owner).safeMintGeneral(owner.address,"1st","1st","1st",0)
   ...
   await expect(market.connect(buyer).offerForNotListedToken(0,0,collection.address,{
     value: ethers.utils.parseUnits("50.0","ether")
   })).to.emit(market,"OfferForNotListed")
   ...
   await collection.connect(owner).setApprovalForAll(
    ...
    expect(await market.connect(owner).denyOfferForNotListedToken(0)).to.emit(market,"OfferForNotListed")
   .to.changeEtherBalance(market,ethers.utils.parseUnits("-50","ether"))
